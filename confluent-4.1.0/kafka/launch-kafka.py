...
role_name = environ.get('ZK_ROLE_NAME')
...
for host in response.json():
    if host["Name"] in lExpectedHosts:
        if list is None:
            list = host["DirectAddress"] + ":22181"
        else:
            list = list + "," + host["DirectAddress"] + ":22181"

print list

cmd = 'export KAFKA_ZOOKEEPER_CONNECT="' + list + '"'
myhost = environ.get('DUPLO_DOCKER_HOST')
cmd1 = 'export KAFKA_ADVERTISED_LISTENERS=PLAINTEXT://' + myhost + ':29092' 
